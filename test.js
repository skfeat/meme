  ...
  fetch("https://meme-api.com/gimme")
    ...
    .then(meme => {
      ...
      if (fetchedUrls.includes(url)) {
        ...
        const div = document.createElement("div");
        div.innerHTML+=`<div style="border:1px solid grey; border-radius: 10px; margin:10px; padding:10px;"><center>`+`<b>Post Title : ${meme["title"]}<br></b>`+`<img src='${url}' width=250px height=250px><br>`+`<b>Posted by ${meme["author"]}</b>`+`</center></div>`
        document.getElementById('test').appendChild(div)
      }
    })
}
